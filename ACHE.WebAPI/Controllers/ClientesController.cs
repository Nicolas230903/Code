...
namespace ACHE.WebAPI.Controllers
{
    public class ClientesController : ApiController
    {
        ...
        public HttpResponseMessage obtener(string token, string filtro, int page, int pageSize)
        {
            try
            {
                ...
                if (idUsuario > 0)
                {
                    ...
                    {
                        var resultado = PersonasCommon.ObtenerPersonas(filtro, "C", page, pageSize, usu);
        ...
        string telefono, string email, int idProvincia, int idCiudad, string provinciaDesc, 
        string ciudadDesc, string domicilio, string pisoDepto, string cp, string obs, int listaPrecio, string codigo, decimal saldoInicial)
        {
            try
            {
                var idUsuario = TokenCommon.validarToken(token);
                if (idUsuario > 0)
                {
                    ...
                    int idPersona = PersonasCommon.GuardarPersonas(id, razonSocial, nombreFantasia, condicionIva, personeria, tipoDoc, 
                        nroDoc, telefono, email, "C", idProvincia, idCiudad, provinciaDesc, ciudadDesc, domicilio, pisoDepto, cp, obs, 
                        listaPrecio, codigo, saldoInicial,0,"",0, usu);
                    return Request.CreateResponse(HttpStatusCode.OK, idPersona.ToString());
                }
                else
                    return Request.CreateErrorResponse(HttpStatusCode.Forbidden, "Token inválido");
            }
            ...
            try
            {
                ...
                if (idUsuario > 0)
                {
                    ...
                    if (PersonasCommon.EliminarPersona(id, usu))
                        ...
                        return Request.CreateResponse(HttpStatusCode.BadRequest, "El cliente no existe");
                }
                else
                    return Request.CreateErrorResponse(HttpStatusCode.Forbidden, "Token inválido");
            }
