...
jQuery(document).ready(function () {
    ...
    //$('.tooltips').tooltip({ trigger: 'hover', placement: 'top' });
...
$(document).ready(function () {
    if ($(window).outerWidth() > 767) {
        $('.navbar-nav > li').on('mouseenter', function () {
            ...
            $(this).children('a').css({
                "border-bottom": "3px solid #2B9B8F",
                "color": "#2B9B8F",
    ...
    // Modificación
    $('.headermenu .handle-dropdown-manually').on('click', function (event) {
        $(this).parent().toggleClass('open');
    });

    var subMenu = $('.headerbar .topnav ul.navbar-nav > li .dropdown-menu li .submenu');

    $('body').on('click', function (e) {
        if (!$('.headermenu .handle-dropdown-manually').parent().is(e.target)
            && $('.headermenu .handle-dropdown-manually').parent().has(e.target).length === 0
            && $('.open').has(e.target).length === 0
        ) {
            $('.headermenu .handle-dropdown-manually').parent().removeClass('open');
        }
    });

    $('.headerbar .topnav ul.navbar-nav > li .dropdown-menu li a.toggle-sub').on('click', function (e) {
        e.preventDefault();
