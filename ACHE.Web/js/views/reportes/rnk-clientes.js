...
function configFilters() {
    ...
    $("#txtFechaDesde, #txtFechaHasta, #txtCiudad").keypress(function (event) {
...
function filtrar() {
    ...
    if ($('#frmSearch').valid()) {
        ...
        var ciudad = ($("#ddlCiudad").val() == "" || $("#ddlCiudad").val() == null) ? "" : $("#ddlCiudad").val();
        var currentPage = parseInt($("#hdnPage").val());

        var info = "{ provincia: '" + $("#ddlProvincia").val()
                    + "', ciudad: '" + ciudad
        ...
        $.ajax({
            type: "POST",
            url: "rnk-clientes.aspx/getResults",
            data: info,
...
function verTodos() {
    $("#txtFechaDesde, #txtFechaHasta, #txtCiudad").val("");
